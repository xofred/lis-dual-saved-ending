...
<header class="site-header">
  ...
  <nav>
    ...
    <button type="button" id="searchToggle" class="nav-search-toggle" aria-label="搜尋全站故事">🔍</button>
  </nav>
</header>
"""
...
# 每頁都放的搜尋面板(靜態,不含資料)。索引資料在 docs/search-index.json,
# 邏輯在 docs/search.js,所以加一篇故事只會動到那份 JSON,不會讓全站頁面重產生。
SEARCH_WIDGET = """
<div class="search-overlay" id="searchOverlay" data-root="{root}" hidden>
  <div class="search-panel">
    <div class="search-panel-head">
      <input type="text" id="searchInput" class="search-input" placeholder="搜尋全站故事……" autocomplete="off" spellcheck="false">
      <button type="button" id="searchClose" class="search-close" aria-label="關閉搜尋">✕</button>
    </div>
    <div class="search-status" id="searchStatus">輸入關鍵字,搜遍全站{count}篇故事的正文。</div>
...
# docs/search.js 的內容:全站全文搜尋。索引資料獨立放在 docs/search-index.json,
# 第一次打開搜尋面板才會抓(不拖慢首次進站的載入),抓過一次靠 Service Worker 快取。
SEARCH_JS = """/* 由 build.py 產生:全站全文搜尋邏輯,索引資料另外放在 search-index.json */
(function() {
...
def render_search(root, count):
    """每頁都放的搜尋面板(靜態片段,不含索引資料)。"""
    return SEARCH_WIDGET.replace("{root}", root).replace("{count}", str(count))


def render_search_js():
    """docs/search.js 的內容:全文搜尋邏輯,索引資料另外放 search-index.json。"""
    return SEARCH_JS


def render_search_index(entries):
    """docs/search-index.json 的內容。entries 是 build.py 已經組好的
    [{"title", "slug", "meta", "text"}, ...],這裡只負責序列化。不預先存一份
    小寫化的搜尋欄位(內文本來就有兩三千字,存兩份等於檔案體積直接翻倍)——
    小寫化留給前端在第一次搜尋時對每筆資料做一次、就地記住,不重複算。"""
    data = [
        {"title": e["title"], "slug": e["slug"], "meta": e["meta"], "text": e["text"]}
        for e in entries
    ]
...
#   - CSS / player.js / search.js / search-index.json → StaleWhileRevalidate
#     (內容會隨新章節/新歌變動,不能像圖片那樣一路 CacheFirst 到底)
