...
`build.py` 最上面的 `SECTIONS` 就是**第一季**的分區設定，**新增章節後要手動更新這裡的數字範圍**：
...
SEASON_2 = { "name": "波特蘭", "desc": "..." }   # 第二季只有一個分區,名稱與簡介寫死在這
```

每一項是 `(分區名稱, 起始編號, 結束編號, 分區描述)`，系統按章節編號自動歸類。**這是目前唯一需要手動維護的地方。**
...
第二季在視覺上明顯區隔：上方一條 `season-divider`（雙線分隔 + 「SEASON TWO」+ 大標題），下方的區塊 `.season-block` 有暖色底色，且**預設展開**（篇數少）。結構跟其他分區一模一樣，之後第二季篇數多了，把 `render_section(..., is_open=True)` 改成預設收合即可。
