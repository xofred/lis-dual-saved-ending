...
ordered/                       ← 第一季章節原始檔資料夾（輸入）
├── 001_xxx.md
├── 002_xxx.md
└── ...                        ← 每個檔案開頭必須是 "# 章節標題" 這一行

ordered_s2/                    ← 第二季章節原始檔（同樣規則，編號各自從 001 起算）
├── 001_xxx.md
└── ...
...
1. **檔名格式**：`{三位數編號}_{英文slug}.md`，例如 `001_dual_saved_ending_family_reunion.md`
   ...
   - **要在某個編號的位置插入新故事、又不想把後面所有編號整批往後推**：數字後面加一個小寫字母當後綴，例如 `002a_interrogation_flashback.md`、`002b_jefferson_verdict.md`——兩篇都算「第 2 章」這個位置，`002` 排最前面、`002a` 再來、`002b` 接著，字母順序決定先後。頁面跟卡片會顯示成「第 002a 章」「002b」這樣。篇章分區（`SECTIONS`）的數字範圍判斷只看數字本身，不看字母，所以插入的章節會自動落在對的分區裡，不用調整範圍
   - **同一季裡兩個檔案不小心取了一樣的編號**（純手誤，不是上面那種刻意插入）：`build.py` 建置時會自動比對、印出黃色警告告訴你是哪個編號、哪幾個檔案撞在一起——不會讓建置失敗（畢竟配對靠的是 slug 不是編號），純粹是提醒，你可以決定要不要用上面的字母後綴法把其中一個改掉
...
5. **不要在正文裡寫原始 HTML 標籤**（`<script>`、`<iframe>` 之類）。Markdown 的標準行為是原始 HTML 會原封不動穿透進最終頁面，不會自動跳脫——`build.py` 會在讀進每篇章節時掃一遍（`scan_dangerous_html()`），抓到 `<script>`/`<iframe>`/`<svg>` 這類危險標籤、`onerror=` 這類行內事件屬性、或 `javascript:` 協定，就直接讓建置失敗、印出是哪個檔案第幾行。故事草稿常常是從別處貼進來的，這一關是防萬一不小心夾帶了這種東西，會在公開網站上被瀏覽器真的執行。誤判的話（例如你真的需要在正文裡討論這些語法本身），去 `build.py` 的 `DANGEROUS_HTML_PATTERNS` 調整。
...
**增量建置：沒改過的圖片不會重新壓。** 壓縮很吃 CPU（全部重跑一次要接近一分鐘），`build.py` 會在 `site_build/.image_cache.json`（不進版本控制，純本機加速用）記住每個輸出檔案上次是拿哪個來源檔案的內容雜湊（sha1）、用什麼參數壓的；只要來源檔案內容沒變、壓縮參數也沒變，就直接沿用上次壓好的檔案，不重新跑一次 Pillow。改壓縮參數（`IMAGE_MAX_DIM` 等常數）會自動讓對應的快取失效；如果之後改了 `resize_image()` 的壓縮邏輯本身，記得把 `RESIZE_ALGO_VERSION` 手動 +1，讓所有快取一次性失效重新壓一輪。改名或刪掉的素材，原本靠整批清空資料夾防止孤兒檔案殘留，現在這三個資料夾（`docs/images/`、`docs/thumbs/polaroids/`、`docs/thumbs/journal/`）跨次建置保留內容，孤兒清理改用 `prune_orphans()` 逐一比對清掉，效果一樣。實測：第一次（冷快取）跑完整個網站約 50 秒，之後只加一張新素材，重跑降到 4 秒左右。
